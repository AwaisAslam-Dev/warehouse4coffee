...
const ExploreMenu = () => {
  ...
  return (
    <section 
      ref={sectionRef}
      id="explore-menu"
      className="relative py-20 md:py-28 lg:py-32 overflow-hidden bg-linear-90-to-br from-[#FFF8F0] via-[#FDF5E6] to-[#F5E6D3]"
    >
      
      {/* Background Decorative Elements */}
      <div className="absolute inset-0 opacity-30">
        <div className="absolute top-20 left-10 w-64 h-64 bg-[#C7A252]/10 rounded-full blur-3xl animate-pulse" />
        <div className="absolute bottom-20 right-10 w-80 h-80 bg-[#D4A574]/10 rounded-full blur-3xl animate-pulse delay-1000" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 md:mb-16 lg:mb-20 opacity-0 translate-y-10 transition-all duration-700 ease-out"
             ref={(el) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {menuCategories.map((category, index) => (
            <div
              ...
              className="opacity-0 translate-y-10 transition-all duration-700 ease-out hover:z-10"
              style={{ transitionDelay: `${index * 100}ms` }}
            >
              {/* Glassmorphic Card */}
              <div className="relative h-full bg-white/30 backdrop-blur-md rounded-2xl overflow-hidden shadow-xl transition-all duration-500 hover:shadow-2xl hover:scale-105 border border-white/40 cursor-pointer group">
                ...
                {/* Image Container */}
                <div className={`relative h-56 overflow-hidden bg-linear-to-br ${category.bgColor}`}>
                  <img 
                    src={category.image}
                    alt={category.title}
                ...
                <div className="p-6 text-center">
                  ...
                  <div className="flex items-center justify-center gap-3 mb-4">
                    <div className="p-2 rounded-xl bg-linear-to-br from-[#8B5E3C]/10 to-[#C7A252]/10 border border-[#C7A252]/20 group-hover:scale-110 transition-transform duration-300">
                      <category.icon className="w-5 h-5 text-[#8B5E3C]" />
                    </div>
                    <h3 className="font-serif text-xl font-bold text-[#2C1810] group-hover:text-[#8B5E3C] transition-colors duration-300">
                      {category.title}
                    </h3>
                  </div>
                  ...
                  <button 
                    ...
                    className="group/btn w-full mt-2 px-6 py-2.5 bg-linear-to-r from-[#8B5E3C] to-[#C7A252] rounded-full font-semibold text-white text-sm transition-all duration-300 hover:shadow-lg hover:scale-105 flex items-center justify-center gap-2"
                  >
                    <span>Order Now</span>
        ...
        {/* Bottom CTA */}
        <div className="text-center mt-12 md:mt-16 opacity-0 translate-y-10 transition-all duration-700 ease-out delay-500"
             ref={(el) => {
               if (el) {
                 const observer = new IntersectionObserver(
                   (entries) => {
          ...
          <button 
            ...
            className="group px-8 py-3 bg-linear-to-r from-[#8B5E3C] to-[#C7A252] rounded-full font-semibold text-white text-base hover:shadow-xl transition-all duration-300 hover:scale-105 inline-flex items-center gap-2"
          >
